...
export default function MintCanvas({ fallbackPriceEth = 0.003, onMintSuccess }: MintCanvasProps) {
  ...
  const maxQuantity = useMemo(() => {
    if (mode === "whitelist") return wlRemainingForMe !== undefined ? Number(wlRemainingForMe as bigint) : 0;
  ...
  useEffect(() => {
    ...
    void processConfirmedReferralMint(address, hash)
      .then((result) => {
        if (result.minterPoints) pushToast({ title: "Points earned", description: `You earned ${result.minterPoints.toLocaleString()} referral points.`, variant: "success" });
      })
  ...
  const displayMaxSupply = NFT_PUBLIC_SUPPLY;
  ...
  const isMintButtonDisabled = !isConnected || isPending || isSimulatePending || receipt.isLoading || isSoldOut || isNotOpenYet || hasInsufficientFundsError || Boolean(simulateError) || mintSent || maxQuantity <= 0 || totalPriceWei === undefined;
...
function remainingLabel(mintedCount: number | undefined) {
  if (mintedCount === undefined) return "";
  return `${Math.max(NFT_PUBLIC_SUPPLY - mintedCount, 0).toLocaleString()} remaining`;
}*/
...
export default function MintCanvas({ fallbackPriceEth = 0.003, onMintSuccess }: MintCanvasProps) {
  ...
  const { data: isMintOpen } = useReadContract({ ...contract, functionName: "openMint" });
  const { data: wlDeadline } = useReadContract({ ...contract, functionName: "wlDeadline" });
  const { data: publicPrice } = useReadContract({ ...contract, functionName: "publicMintPrice" });
  const { data: wlPrice } = useReadContract({ ...contract, functionName: "wlMintPrice" });
  const { data: totalMintedSoFar, refetch: refetchTotalMinted } = useReadContract({ ...contract, functionName: "totalSupply" });
  const { data: publicAvailableOnChain, refetch: refetchPublicAvailable } = useReadContract({ ...contract, functionName: "publicAvailable" });
  const { data: publicWalletCap } = useReadContract({ ...contract, functionName: "publicWalletCap" });
  const { data: publicMintedByMe, refetch: refetchPublicMintedByMe } = useReadContract({ ...contract, functionName: "publicMinted", args: address ? [address] : undefined, query: { enabled: Boolean(address) } });
  ...
  // Use WL price when in whitelist mode, public price otherwise
  const priceWei = (mode === "whitelist" ? wlPrice : publicPrice) as bigint | undefined;
  const totalPriceWei = priceWei !== undefined ? priceWei * BigInt(quantity) : undefined;
  const displayPriceEth = priceWei !== undefined ? Number(formatEther(priceWei)) * quantity : fallbackPriceEth * quantity;
  const formattedPrice = displayPriceEth >= 1 ? displayPriceEth.toFixed(1) : displayPriceEth.toFixed(4);
  ...
  const displayMaxSupply = maxSupplyOnChain !== undefined ? Number(maxSupplyOnChain as bigint) : undefined;
  const mintProgress = mintedCount !== undefined && displayMaxSupply !== undefined ? Math.min(mintedCount / displayMaxSupply, 1) : 0;
  const isSoldOut = mintedCount !== undefined && displayMaxSupply !== undefined && mintedCount >= displayMaxSupply;
  const isNotOpenYet = isMintOpen === false;
  const isMintButtonDisabled = !isConnected || isPending || isSimulatePending || receipt.isLoading || isSoldOut || isNotOpenYet || hasInsufficientFundsError || Boolean(simulateError) || mintSent || maxQuantity <= 0 || totalPriceWei === undefined;
  const buttonLabel = mintSent ? "Minted" : isSoldOut ? "Sold out" : isNotOpenYet ? "Mint not open" : isPending ? "Confirm in wallet…" : receipt.isLoading ? "Confirming…" : maxQuantity <= 0 ? (mode === "whitelist" ? "Whitelist allocation used" : "Public cap reached") : `Mint ${quantity > 1 ? `${quantity} canvases` : "canvas"} for ${formattedPrice} ETH`;
  ...
  return (
    <div>
      ...
      {canUseWhitelist && countdown && <p className="mt-3 text-right font-[family-name:var(--font-geist-mono)] text-xs text-[var(--foreground-muted)]">Whitelist window: {countdown}</p>}
      <motion.button whileHover={{ y: -1 }} whileTap={{ scale: 0.98 }} disabled={isMintButtonDisabled} onClick={handleMint} className="quiet-button quiet-button--filled mt-6 w-full py-3 text-sm">{buttonLabel}</motion.button>
      <div className="mt-5 border-t border-[var(--border-hairline)] pt-4"><div className="h-1 w-full overflow-hidden bg-[var(--border-hairline)]"><motion.div className="h-full bg-[var(--accent)]" initial={{ width: 0 }} animate={{ width: `${mintProgress * 100}%` }} transition={{ duration: 0.4, ease: "easeOut" }} /></div><div className="mt-3 flex items-center justify-between font-[family-name:var(--font-geist-mono)] text-xs text-[var(--foreground-muted)]"><span>{mintedCount?.toLocaleString() ?? "—"} / {displayMaxSupply?.toLocaleString() ?? "—"} minted</span><span>{remainingLabel(mintedCount, displayMaxSupply)}</span></div></div>
      {!isConnected && <p className="mt-3 text-sm text-[var(--accent)]">Connect your wallet to mint.</p>}
      {hasInsufficientFundsError && <p className="mt-3 text-sm text-[#c17a72]">Insufficient funds for gas and mint price.</p>}
      <ReferralControls address={address} />
    </div>
...
function remainingLabel(mintedCount: number | undefined, maxSupply: number | undefined) {
  if (mintedCount === undefined || maxSupply === undefined) return "";
  return `${Math.max(maxSupply - mintedCount, 0).toLocaleString()} remaining`;
}